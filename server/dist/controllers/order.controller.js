...
exports.getOrdersByUserId = exports.hasUserPurchasedProduct = exports.getOrderById = exports.getActiveOrders = exports.markOrderAsDeleted = exports.getOrders = exports.updateOrder = exports.addOrder = exports.checkout = exports.checkStockAvailability = void 0;
...
exports.checkStockAvailability = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!cart || !cart.items || cart.items.length === 0) {
        throw new ApiError_1.ApiError(400, "Cart is empty or not found.");
    }
    for (const item of cart.items) {
        const product = item.product;
        if (!product || (product.stock || 0) < item.quantity) {
            throw new ApiError_1.ApiError(400, `${product ? product.productName : "Product"}'s ${item.quantity} quantity is not available in stock.`);
        }
    }
    res.status(200).json(new ApiResponse_1.ApiResponse(200, null, "All products are available in sufficient quantity.").toJSON());
});
exports.checkout = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (promoCodeId) {
        const offer = await data_source_1.AppDataSource.getRepository(Offer_1.Offer).findOneBy({ id: promoCodeId });
        if (!offer || !isActive(offer.startDate, offer.endDate)) {
            throw new ApiError_1.ApiError(400, "Invalid or inactive promo code.");
        }
    ...
    const result = await data_source_1.AppDataSource.transaction(async (transactionalEntityManager) => {
        const orderRepository = transactionalEntityManager.getRepository(Order_1.Order);
        const orderItemRepository = transactionalEntityManager.getRepository(OrderItem_1.OrderItem);
        const productRepository = transactionalEntityManager.getRepository(Product_1.Product);
        const cartItemRepository = transactionalEntityManager.getRepository(CartItem_1.CartItem);
    ...
    res.status(201).json(new ApiResponse_1.ApiResponse(201, result, "Checkout completed successfully.").toJSON());
});
// Add a new order with multiple products
exports.addOrder = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!products || products.length === 0) {
        throw new ApiError_1.ApiError(400, "At least one product is required.");
    }
    ...
    for (const product of products) {
        ...
        if (!foundProduct) {
            throw new ApiError_1.ApiError(400, `Product with id ${productId} not found.`);
        }
    ...
    // Save order items
    await orderItemRepository.save(orderItems);
    res.status(201).json(new ApiResponse_1.ApiResponse(201, savedOrder, "Order created successfully.").toJSON());
});
exports.updateOrder = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    await orderItemRepository.save(orderItems);
    res.status(200).json(new ApiResponse_1.ApiResponse(200, updatedOrder, "Order updated successfully.").toJSON());
});
...
exports.getOrders = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    res.status(200).json(new ApiResponse_1.ApiResponse(200, results, "Orders fetched successfully", {
        total,
        page: parsedPage,
...
exports.markOrderAsDeleted = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const orderId = req.params.orderId;
    const customReq = req;
    const orderRepository = data_source_1.AppDataSource.getRepository(Order_1.Order);
    ...
    res.status(200).json(new ApiResponse_1.ApiResponse(200, null, "Order marked as deleted.").toJSON());
});
...
exports.getActiveOrders = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    res.status(200).json(new ApiResponse_1.ApiResponse(200, formattedOrders, "Active orders fetched successfully").toJSON());
});
...
exports.getOrderById = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!orderItems || orderItems.length === 0) {
        throw new ApiError_1.ApiError(404, "No order items found.");
    }
    ...
    res.status(200).json(new ApiResponse_1.ApiResponse(200, { order: formattedOrder, orderItems: formattedOrderItems }, "Order details fetched successfully").toJSON());
});
exports.hasUserPurchasedProduct = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.params.userId;
    ...
    if (orderIds.length === 0) {
        return res.status(200).json(new ApiResponse_1.ApiResponse(200, { purchased: false }, "User has not purchased product").toJSON());
    }
    const orderItemRepository = data_source_1.AppDataSource.getRepository(OrderItem_1.OrderItem);
    // Check if any order items match the productId and belong to those orders
    const orderItem = await orderItemRepository.findOne({
        where: {
            orderId: (0, typeorm_1.In)(orderIds),
            productId,
        },
    });
    const hasPurchased = !!orderItem;
    res.status(200).json(new ApiResponse_1.ApiResponse(200, { purchased: hasPurchased }, "Purchase check completed").toJSON());
});
exports.getOrdersByUserId = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.params.userId;
    const customReq = req;
    if (customReq.user.id !== userId && customReq.user.role !== "Admin") {
        throw new ApiError_1.ApiError(403, "Unauthorized access");
    ...
    res.status(200).json(new ApiResponse_1.ApiResponse(200, formattedOrders, "User orders fetched successfully").toJSON());
});
